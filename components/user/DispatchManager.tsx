...
export const DispatchManager: React.FC<Props> = ({ data, onUpdate }) => {
  ...
  const handleRepeatOrder = (d: DispatchEntry) => {
      ...
      // Clone rows but reset quantities to 0 for a new order
      ...
      setActiveDispatch({
          date: new Date().toISOString().split('T')[0],
          dispatchNo: '', // Will be auto-filled
          status: DispatchStatus.PENDING,
          partyId: d.partyId,
          rows: newRows
      });
                        ...
                        <div key={i} className="group flex justify-between items-center text-xs bg-slate-50 px-3 py-2 rounded-lg border border-slate-100 hover:border-indigo-200 transition-colors">
                            <div className="flex flex-col">
                                ...
                                <div className="flex gap-2 text-[10px] font-bold text-slate-600">
                                    <span>Disp: {row.weight.toFixed(3)}</span>
                                    {row.productionWeight && row.productionWeight > 0 ? (
                                        <span className="text-indigo-600">Prod: {row.productionWeight.toFixed(3)}</span>
                                    ) : null}
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                           ...
                           {isExpanded && (
                             <div className="bg-slate-50/80 border-t border-slate-100 animate-in slide-in-from-top-2 duration-300">
                                <div className="px-4 py-2 bg-white border-b border-slate-100 flex justify-between items-center">
                                    <button onClick={() => handleRepeatOrder(d)} className="bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-200 px-3 py-1.5 rounded text-xs font-bold flex items-center gap-1 shadow-sm transition-colors">
                                ...
                                <div className="flex justify-end px-4 py-1">
                                    <button onClick={(e) => toggleToday(e, d)} className={`text-[9px] font-bold px-2 py-1 rounded border transition-colors ${isToday ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-500'}`}>
                                        ...
                                        {d.rows.map(row => {
                                            ...
                                            return (
                                               <tr key={row.id} className="hover:bg-indigo-50/20 transition-colors">
                                                  ...
                                                  <td className="px-3 py-1 text-right font-mono font-bold text-slate-800"><input type="number" value={row.weight === 0 ? '' : row.weight.toFixed(3)} onChange={(e) => handleRowUpdate(d, row.id, 'weight', parseFloat(e.target.value) || 0)} className="w-full text-right bg-transparent font-mono font-bold text-slate-800 outline-none border-b border-transparent focus:border-indigo-300 transition-colors py-1" /></td>
                                                  <td className="px-3 py-1 text-right font-mono font-bold text-indigo-600"><input type="number" value={row.productionWeight === 0 ? '' : row.productionWeight?.toFixed(3)} placeholder="-" onChange={(e) => handleRowUpdate(d, row.id, 'productionWeight', parseFloat(e.target.value) || 0)} className="w-full text-right bg-transparent font-mono font-bold text-indigo-600 outline-none border-b border-transparent focus:border-indigo-300 transition-colors py-1" /></td>
