...
export const SlittingDashboard: React.FC<Props> = ({ data }) => {
  ...
  const handleGridChange = (rowId: string, field: 'grossWeight' | 'coreWeight', value: string) => {
     ...
     // Check if we are editing the first row's core weight
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8 animate-in fade-in zoom-in duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activeJobs.map((job, idx) => {
            ...
            const statusColor = job.status === 'IN_PROGRESS' ? 'border-l-blue-500' : 'border-l-slate-300';
            const badgeClass = job.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-700 animate-pulse ring-2 ring-blue-200' : 'bg-slate-100 text-slate-500';
                ...
                >
                   <div className="p-6 relative">
                       ...
                       <div className="w-full bg-slate-100 rounded-full h-2 mb-2 overflow-hidden shadow-inner relative">
                           <div 
                                className={`h-full rounded-full transition-all duration-1000 ease-out ${job.status==='IN_PROGRESS' ? 'bg-gradient-to-r from-blue-400 to-blue-600 shadow-[0_0_10px_rgba(59,130,246,0.5)]' : 'bg-slate-300'}`} 
          ...
          {activeJobs.length === 0 && (
            <div className="col-span-full py-20 text-center bg-white rounded-3xl border border-dashed border-slate-300 shadow-sm animate-in fade-in duration-700">
                ...
                <h3 className="text-xl text-slate-800 font-bold mb-2">All Caught Up!</h3>
                <p className="text-slate-400 font-medium">No pending jobs assigned.</p>
            </div>
          )}
        </div>
      </div>
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6 relative z-10">
             <div>
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md border uppercase tracking-wider ${selectedJob.status === 'IN_PROGRESS' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden relative z-0 animate-in fade-in slide-in-from-bottom-8 duration-500">
         ...
         <div className="overflow-x-auto max-h-[60vh]">
            <table className="w-full text-xs text-left">
               <thead className="bg-slate-50/80 backdrop-blur text-slate-500 font-bold uppercase border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                  <tr>
                     <th className="px-3 py-4 text-center w-12">#</th>
                     <th className="px-3 py-4 text-right text-slate-800 w-28">Gross</th>
                     <th className="px-3 py-4 text-right text-red-500 w-24">Core</th>
                     <th className="px-3 py-4 text-right text-emerald-600 w-28">Net</th>
                     <th className="px-3 py-4 text-right w-24">Meter</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-slate-100">
                  {gridRows.map((row) => (
                     <tr key={row.id} className="hover:bg-indigo-50/30 transition-colors h-12 group">
                        <td className="px-3 text-center font-mono text-slate-400 bg-slate-50/30 font-medium">{row.srNo}</td>
                        <td className="px-2">
                            <input 
                                type="number" 
                                step="0.001"
                                ...
                                className="w-full text-right font-mono font-bold text-slate-700 bg-slate-100 focus:bg-white border-2 border-transparent focus:border-indigo-500 rounded-lg py-2 px-3 outline-none transition-all shadow-sm focus:shadow-md"
                                placeholder="0.000"
                            />
                        </td>
                        <td className="px-2">
                            <input 
                                ...
                                className="w-full text-right font-mono font-bold text-red-500 bg-red-50/30 focus:bg-white border-2 border-transparent focus:border-red-400 rounded-lg py-2 px-3 outline-none transition-all shadow-sm focus:shadow-md placeholder-red-200"
                                placeholder="0.000"
                            />
                        </td>
                        <td className="px-3 text-right">
                            <span className={`font-mono font-bold text-base ${row.netWeight > 0 ? 'text-emerald-600' : 'text-slate-300'}`}>
                                {row.netWeight > 0 ? row.netWeight.toFixed(3) : '-'}
                            </span>
                        </td>
                        <td className="px-3 text-right">
                            <span className={`font-mono font-bold ${row.meter > 0 ? 'text-slate-600' : 'text-slate-300'}`}>
         ...
         <div className="p-4 sm:p-5 bg-white border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4 sticky bottom-0 z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
             <div className="grid grid-cols-2 gap-3 w-full sm:w-auto flex-1 max-w-lg">
                 <button onClick={addMoreRows} className="h-12 bg-white text-indigo-600 border-2 border-indigo-100 hover:border-indigo-200 hover:bg-indigo-50 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-2 group">
                     <span className="bg-indigo-100 text-indigo-600 rounded-full w-5 h-5 flex items-center justify-center text-lg leading-none group-hover:bg-indigo-200">+</span>
                     Add Rows
                 </button>
                 
                 <div className="h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 flex flex-col justify-center items-end shadow-inner">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Current Size Total</span>
                    <span className="text-sm font-bold text-emerald-600">{currentCoilTotal.toFixed(3)} <span className="text-[10px] text-emerald-400 font-normal">kg</span></span>
                 </div>
             </div>
             ...
             <button 
                ...
                className={`h-12 w-full sm:w-auto px-8 rounded-xl font-bold text-sm shadow-lg transition-all transform active:scale-95 flex items-center justify-center gap-2 ${hasUnsavedChanges || isAutoSaving ? 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-200' : 'bg-emerald-50 text-emerald-600 border border-emerald-200 cursor-default opacity-100 shadow-none'}`}
             >
                {isAutoSaving ? (
                    <>
