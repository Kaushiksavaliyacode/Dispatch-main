    ...
    <div className="space-y-4 md:space-y-6">
      <div className="bg-white rounded-2xl md:rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
          <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 px-4 py-3 md:px-6 md:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
             <div className="flex items-center gap-2 md:gap-3 text-white w-full md:w-auto">
                <div className="p-1.5 md:p-2 bg-white/20 rounded-lg backdrop-blur-sm">
                   <svg className="w-4 h-4 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                </div>
                <div>
                  <h3 className="text-sm md:text-lg font-bold tracking-wide">Master Data</h3>
                  <p className="text-[10px] md:text-xs text-emerald-100 font-medium">Complete History</p>
                </div>
             </div>
             ...
             <div className="flex gap-2 w-full md:w-auto">
                ...
                <button 
                  onClick={downloadCSV}
                  className="bg-white text-emerald-700 hover:bg-emerald-50 px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-xs md:text-sm font-bold flex items-center gap-2 transition-colors shadow-sm"
                >
                  <svg className="w-3 h-3 md:w-4 md:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                ...
                {filteredRows.map((row, index) => {
                  ...
                  return (
                    <tr key={`${row.dispatchId}-${index}`} className="hover:bg-indigo-50/40 transition-colors">
                      ...
                      <td className="px-2 py-1 md:px-6 md:py-3 font-bold text-slate-800">{row.party}</td>
                      <td className="px-2 py-1 md:px-6 md:py-3 font-semibold text-slate-700">{row.size}</td>
                      <td className="px-2 py-1 md:px-6 md:py-3 text-right font-mono text-slate-600">{row.weight.toFixed(3)}</td>
                      <td className="px-2 py-1 md:px-6 md:py-3 text-right font-mono text-slate-600">{row.pcs} <span className="text-[9px] md:text-xs text-slate-400">{isMm ? 'R' : 'P'}</span></td>
                      <td className="px-2 py-1 md:px-6 md:py-3 text-center text-slate-600 font-medium">{row.bundle}</td>
                      <td className="px-2 py-1 md:px-6 md:py-3 text-center">
                         <span className={`px-1.5 py-0.5 md:px-2 md:py-1 rounded text-[9px] md:text-[10px] font-bold tracking-wide ${statusColor}`}>
